...
from models import AlexNet_Complex
...
def to_complex(x):
    ...
    return x.view(dtype=np.complex128) # 1D now

# Datasets
train_dataset = CLFM_Dataset(".", train=True, is_complex=True)
val_dataset = CLFM_Dataset(".", train=False, is_complex=True)
test_dataset = CLFM_Dataset(".", train=False, test=True, is_complex=True)
...
net = AlexNet_Complex(classes=10).to(device=device)
